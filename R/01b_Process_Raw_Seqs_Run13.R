...
##  This script processes "run13" - 1912KMI-0013         ##
##                                                       ##
...
path <- "./Data/Sonneratia_alba/Seqs/run13" 
...
saveRDS(out,"./Output/run13/out.RDS") # save object
out <- readRDS("./Output/run13/out.RDS") # reload point

filtpath <- file.path(path,"filtered")

# reassign filts for any potentially lost samples
filtFs <- list.files("./Data/Sonneratia_alba/Seqs/run13/filtered", pattern = "_F_filt", full.names = TRUE)
filtRs <- list.files("./Data/Sonneratia_alba/Seqs/run13/filtered", pattern = "_R_filt", full.names = TRUE)
...
saveRDS(errF,"./Output/run13/errF.RDS") # save object
saveRDS(errR,"./Output/run13/errR.RDS") # save object
errF <- readRDS("./Output/run13/errF.RDS") # reload point
errR <- readRDS("./Output/run13/errR.RDS") # reload point
...
saveRDS(mergers,"./Output/run13/mergers.RDS")
...
saveRDS(seqtab,"./Output/run13/seqtab.RDS") # save object
dim(seqtab)
...
# Save progress
saveRDS(seqtab.nochim,"./Output/run13/seqtab.nochim.RDS")
seqtab.nochim = readRDS("./Output/run13/seqtab.nochim.RDS")
...
saveRDS(taxa,"./Output/run13/taxa.RDS")

taxa <- addSpecies(taxa, "./Taxonomy/silva_species_assignment_v138.fa.gz")
saveRDS(taxa,"./Output/run13/taxa_with_spp.RDS")
...
# save it
saveRDS(ps, "./Output/run13/raw_ps_object.RDS")
...
saveRDS(ps.noncontam, "./Output/run13/noncontam_ps_object.RDS")
