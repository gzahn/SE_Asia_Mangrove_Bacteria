...
##  This script processes "run6" - 1912KMI-0006          ##
##                                                       ##
...
path <- "./Data/Avicenia_alba/Seqs/run6" 
...
saveRDS(out,"./Output/run6/out.RDS") # save object
out <- readRDS("./Output/run6/out.RDS") # reload point

filtpath <- file.path(path,"filtered")

# reassign filts for any potentially lost samples
filtFs <- list.files("./Data/Avicenia_alba/Seqs/run6/filtered", pattern = "_F_filt", full.names = TRUE)
filtRs <- list.files("./Data/Avicenia_alba/Seqs/run6/filtered", pattern = "_R_filt", full.names = TRUE)
...
saveRDS(errF,"./Output/run6/errF.RDS") # save object
saveRDS(errR,"./Output/run6/errR.RDS") # save object
errF <- readRDS("./Output/run6/errF.RDS") # reload point
errR <- readRDS("./Output/run6/errR.RDS") # reload point
...
saveRDS(mergers,"./Output/run6/mergers.RDS")
...
saveRDS(seqtab,"./Output/run6/seqtab.RDS") # save object
dim(seqtab)
...
# Save progress
saveRDS(seqtab.nochim,"./Output/run6/seqtab.nochim.RDS")
seqtab.nochim = readRDS("./Output/run6/seqtab.nochim.RDS")
...
saveRDS(taxa,"./Output/run6/taxa.RDS")

taxa <- addSpecies(taxa, "./Taxonomy/silva_species_assignment_v138.fa.gz")
saveRDS(taxa,"./Output/run6/taxa_with_spp.RDS")
...
# save it
saveRDS(ps, "./Output/run6/raw_ps_object.RDS")
...
saveRDS(ps.noncontam, "./Output/run6/noncontam_ps_object.RDS")
